...
struct String : public CXString
{
    inline operator const char* () const { return clang_getCString(*this); }

    String(CXString s) : CXString(s) {}
...
struct CompileCmd : public pimpl_handle< CXCompileCommands >
{
    ...
    ~CompileCmd()
    {
        if(m_handle)
        {
            clang_CompileCommands_dispose(m_handle);
            m_handle = nullptr;
        }
    }
};
